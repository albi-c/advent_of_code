...
def find_best(graph: Graph[Valve], curr: Valve, time: int, curr2: Valve, time2: int) -> int:
    if time <= 1 or time2 <= 1:
        return 0
    ...
    for _, valve in graph.items():
        ...
        if time >= time2:
            ...
            best = max(best, valve.flow * rest + find_best(graph, valve, rest, curr2, time2))

        if time2 > time or (time == time2 and curr != curr2):
            rest = time2 - 1 - curr2.connections[valve]
            best = max(best, valve.flow * rest + find_best(graph, curr, time, valve, rest))
...
advent.solution(find_best(graph_, graph_["AA"], 26, graph_["AA"], 26))
