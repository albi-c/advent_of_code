...
def store_diagnostic(x: int):
    global diagnostic_code
    assert diagnostic_code == 0, "Diagnostic failed"
    diagnostic_code = x


vm = IntcodeVM(code.copy(), (1,), store_diagnostic)
vm.run()
print(diagnostic_code)

diagnostic_code = 0

vm = IntcodeVM(code.copy(), (5,), store_diagnostic)
vm.run()
print(diagnostic_code)
